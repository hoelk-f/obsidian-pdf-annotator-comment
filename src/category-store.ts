...
export class CategoryStore {
  ...
  revision = 0;
  constructor(definitions = defaultCategories(), private persist: (items: CategoryDefinition[]) => Promise<void> = () => Promise.resolve()) {
  ...
  async save(definitions: CategoryDefinition[], expectedRevision: number) {
    ...
    try {
      ...
      this.definitions = next; this.revision++;
